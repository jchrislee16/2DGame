...
public class Control extends GameState {
	...
	private String[] options = { "Up", "Down", "Left", "Right","Attack","Back to Menu" };
	private Color titleColor;
	private Font titleFont;
	private Font font;

	public Control(GameStateManager gsm) {
		this.gsm = gsm;
		try {
	...
	public void draw(Graphics2D g) {
		...
		g.setFont(titleFont);
		int length = (int) g.getFontMetrics().getStringBounds("Control", g).getWidth();
		g.drawString("Control", gsm.getWidth() / 2 - length / 2, gsm.getHeight() / 4);
		// menu options
		g.setFont(font);
